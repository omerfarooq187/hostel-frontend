...
export default function StudentsPage() {
  ...
  const [isUpdatingSecurityFee, setIsUpdatingSecurityFee] = useState(false);
    ...
    try {
      ...
      showError(
        ...
        err.response?.data?.message ||
          "Unable to load students and users. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchTotalCollection = async (studentId) => {
    try {
      ...
      console.error(
        "Failed to fetch total collection for student",
        studentId,
        err
      );
    }
  ...
  // NEW: Fetch security fee for a student
  const fetchStudentSecurityFee = async (studentId) => {
    try {
      const res = await api.get(
        `/api/admin/allocations/student/${studentId}/security-fee`
      );
      return res.data.securityFee;
    } catch (err) {
      console.error("Failed to fetch security fee for student", studentId, err);
      return null;
    }
  };

  // NEW: Update security fee for a student
  const updateStudentSecurityFee = async (studentId, securityFeeAmount) => {
    try {
      const res = await api.put(
        `/api/admin/allocations/student/${studentId}/security-fee`,
        null,
        {
          params: { securityFee: securityFeeAmount },
        }
      );
      return res.data;
    } catch (err) {
      console.error("Failed to update security fee", err);
      throw err;
    }
  };
  ...
  const filteredAndSortedStudents = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (s) =>
          (s.user?.name?.toLowerCase() || s.name?.toLowerCase()).includes(
            term
          ) ||
  ...
  const stats = useMemo(() => {
    ...
    const allocated = Object.values(allocationsMap).filter(
      (a) => a?.active === true
    ).length;
  ...
  const openEditModal = async (student) => {
    ...
    // Fetch security fee using the dedicated endpoint
    try {
      const securityFeeValue = await fetchStudentSecurityFee(student.id);
      setSecurityFee(securityFeeValue ? securityFeeValue.toString() : "");
    } catch (err) {
      console.error("Failed to fetch security fee:", err);
      setSecurityFee("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingStudentId) {
        ...
        res = await api.put(`/api/admin/students/${editingStudentId}`, updateData);

        // Update security fee using the dedicated endpoint
        if (securityFee && securityFee.trim() !== "") {
          await updateStudentSecurityFee(editingStudentId, parseFloat(securityFee));
        }

        // Refresh student data
        const updatedStudent = await api.get(`/api/admin/students/${editingStudentId}`);
        setStudents(
          students.map((s) => (s.id === editingStudentId ? updatedStudent.data : s))
        );
        ...
        res = await api.post(`/api/admin/students/${selectedUser.id}`, studentData, {
          params: { hostelId },
        });

        // Update security fee using the dedicated endpoint
        if (securityFee && securityFee.trim() !== "") {
          await updateStudentSecurityFee(res.data.id, parseFloat(securityFee));
        }

        // Refresh to get updated student with security fee
        const updatedStudent = await api.get(`/api/admin/students/${res.data.id}`);
        setStudents([...students, updatedStudent.data]);
    ...
    try {
      const res = await api.post(
        "/api/admin/students",
        {
          ...
          guardianPhone: manualGuardianPhone,
        },
        {
          params: { hostelId },
        }
  ...
  const handleDeactivate = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to deactivate this student? They will no longer be active in the system."
      )
    )
      return;
    ...
    try {
      ...
      if (err.response?.status === 409) {
        ...
        showError(
          "Deactivation Failed",
          err.response?.data?.message ||
            "Unable to deactivate student. Please try again."
        );
      }
    }
  };
      ...
      if (response.data.success) {
        ...
        showSuccess(
          ...
          response.data.message ||
            "Student and all related data has been permanently deleted."
        );
      }
    } catch (err) {
  ...
  const handleReactivate = async (id) => {
    ...
    try {
      ...
      showError(
        "Reactivate Failed",
        err.response?.data?.message ||
          "Unable to reactivate student. Please try again."
      );
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 relative"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-blue-800">Total Students</p>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 border border-yellow-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="p-2 bg-yellow-100 rounded-lg">
              <ArchiveBoxXMarkIcon className="h-5 w-5 text-yellow-600" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          {(searchTerm ||
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium text-sm transition-colors"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1200px]">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
              <tr>
                <th
                  className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider cursor-pointer hover:bg-gray-200 w-[18%] min-w-[180px]"
                  onClick={() => toggleSort("name")}
                >
                  Student Details <SortIcon field="name" />
                </th>
                <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[12%] min-w-[120px]">
                  Contact
                </th>
                <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[14%] min-w-[140px]">
                  Guardian
                </th>
                <th
                  className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[9%] min-w-[90px] cursor-pointer hover:bg-gray-200"
                ...
                <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[10%] min-w-[100px]">
                  Security Fee
                </th>
                <th
                  className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[12%] min-w-[120px] cursor-pointer hover:bg-gray-200"
                  onClick={() => toggleSort("collection")}
                >
                  Total Collection <SortIcon field="collection" />
                </th>
                <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs uppercase tracking-wider w-[16%] min-w-[220px]">
                  Actions
                </th>
              </tr>
            </thead>
                    ...
                    >
                      <td className="py-3 px-3">
                        <div>
                          <div
                            ...
                            title={student.user ? student.user.name : student.name}
                          >
                          ...
                          {student.user ? (
                            <>
                              <div
                                className="text-xs text-gray-600 flex items-center gap-1 mt-1 truncate"
                            ...
                            <>
                              <div className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                                <UserPlusIcon className="h-3 w-3 flex-shrink-0" />
                                Manual Admission
                              </div>
                              <div className="text-xs text-gray-600 flex items-center gap-1 mt-1">
                                <IdentificationIcon className="h-3 w-3 flex-shrink-0" />
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-1 text-gray-700 text-sm">
                          ...
                          <span
                            ...
                            title={student.phone || "Not provided"}
                          >
                      ...
                      <td className="py-3 px-3">
                        <div>
                          <div
                            ...
                            title={student.guardianName || "N/A"}
                          >
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-col gap-1">
                          ...
                          >
                            {isAllocated ? "Allocated" : "Not Alloc."}
                          </span>
                        </div>
                      </td>
                      <td className="py-3 px-3 relative group">
                        <div className="flex items-center gap-1 text-gray-700">
                          <CurrencyRupeeIcon className="h-3.5 w-3.5 flex-shrink-0" />
                          <span
                            className="font-medium text-sm truncate"
                            title={
                              student.securityFee
                                ? student.securityFee.toLocaleString()
                                : "Not Set"
                            }
                          >
                            {student.securityFee
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-medium transition-colors duration-200 text-xs whitespace-nowrap"
                            title="Edit Student"
                          >
                          ...
                          {role === "ADMIN" && (
                            <>
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-2.5 py-1 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 rounded-lg font-medium transition-colors duration-200 text-xs whitespace-nowrap"
                                title="Deactivate"
                              >
                              ...
                              <button
                                onClick={() =>
                                  handleHardDelete(student.id, student.name)
                                }
                                className="inline-flex items-center gap-1 px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium transition-colors duration-200 text-xs whitespace-nowrap"
      ...
      <div className="flex justify-center">
        <button
          onClick={fetchData}
          className="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium text-sm transition-colors duration-200"
        >
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg text-sm"
                >
                  OK
                </button>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg text-sm"
                >
                  OK
                </button>
              </div>
...
const Modal = memo(
  ({
    ...
    const availableUsers = users.filter(
      (u) =>
        u.role !== "ADMIN" && !students.some((s) => s.user?.id === u.id)
    );
          ...
          <form onSubmit={onSubmit} className="p-6 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-purple-700">
                      This student was admitted manually. You can update their
                      details.
                    </p>
                  </div>
                </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Security fee amount to be charged for this student
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2.5 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg font-medium text-sm transition-colors"
                disabled={actionLoading}
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg font-medium text-sm transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Enter 13 digits only (e.g., 1234512345671)
            </p>
          </div>
          ...
          <div className="p-4 bg-green-50 rounded-lg border border-green-100">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-700">
                  This student will be created without a user account. They
                  won't have login access. To add login access later, associate
                  them with an existing user using the same CNIC.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg font-medium text-sm transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={actionLoading}
            >
              {actionLoading ? (
                <span className="flex items-center gap-2">
